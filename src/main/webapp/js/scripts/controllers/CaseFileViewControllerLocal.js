...
EDMSApp.controller("CaseFileCtrlLocal",	function($scope, $sce, $http,$filter) {
	var urlBase="/dms/";
    $scope.subDocuments=[];
    $scope.orderReports=[];
	$scope.metadata=[];
	...
	var url = "../../../metadata/caseFiles/CLRE362018_ORDS_2.pdf";
	 DEFAULT_URL = url;
...
/*		if(doc_type=='St. Rep.' || doc_type=='ORD' || doc_type=='Off. Rep.')
    ...
    function findCase(data){
        return data.fd_id == $scope.doc_id;
        }
	
	function getSubDocuments(){
		//$http.get(urlBase+'casefile/getsubdocuments/'+$scope.doc_id).success(function (data) {
            $http.get("../../../metadata/list/caseFiles.json").success(function (data) {
                var temp=[];
                temp=data;
                $scope.casefile=temp.find(findCase);
                console.log($scope.cases);

	    	$scope.subDocuments= $scope.casefile.subDocument;
	    	console.log($scope.subDocuments);
	    	...
	    	angular.forEach($scope.subDocuments, function(value, key) {
	    		  ...
	    		  case'petition':
	    			  ...
	    			  for(var i =0 ;i < $scope.petitions.length -1 ;i++){
	    				  $scope.petitions[i].indexField.if_label ='AMENDMENT PETITION';
                      }
                      
                      $scope.document_name=$scope.petitions[0].sd_document_name;

                      url = "../../../metadata/caseFiles/"+$scope.document_name+".pdf";
                      DEFAULT_URL = url;
...
function findOrder(data){
	...
	function getOrderReports(){
		$http.get("../../../metadata/list/orderReorts.json").success(function (data) {
            ...
            angular.forEach(temp, function(value, key) {
                var sd_id=null;
                if(value[0][0].ord_fd_mid==$scope.doc_id)                
                $scope.orderReports.push(value[0]);
...
/*	function generateReportData(){
	...
	function generateReportData(){
		...
		$scope.ordermodel={'sd_id':value.sd_id,'document_type':value.documentType.at_description,'sd_created_date':value.sd_cr_date,'sd_submitted_date':value.sd_submitted_date,'sd_stamp_date':value.sd_submitted_date,'sd_party':value.sd_party,'sd_nonmaintainable':value.sd_nonmaintainable,'sd_description':value.sd_description,'ord_remark':'','checked':false};
    ...
    function findSubdocument(data){
        return data.sd_id == $scope.sdId;
        }
	$scope.showSubDocument=function(sd_id,$index,doc_type){
       ...
       $scope.sdId=sd_id;
            ...
            $scope.sample= $scope.subDocuments.find(findSubdocument);
            console.log($scope.sample);
	    	DEFAULT_URL='../../../metadata/caseFiles/' +$scope.sample.sd_document_name+".pdf";
