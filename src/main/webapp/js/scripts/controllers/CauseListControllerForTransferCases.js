...
edmsApp.controller("causeListControllerForTransferCases",['$scope','$http','Upload', function($scope,$http,Upload) {
	var urlBase="/dms/";
	...
	function loadMasterData1() {
		var response = $http.get(urlBase+'ecourt/getreportforadditionallistsForTransferCases?causelist_date='+$scope.causelist_date+'&fromCourt='+$scope.fromCourt);
		response.success(function(data, status, headers, config) {		
			$scope.listData= data.modelList;
			console.log("Dataaaaaaaa",$scope.listData.length);
			console.log("Dataaaaaaaa for causelist",$scope.listData);
	...
	function loadMasterData2() {
		var response = $http.get(urlBase+'ecourt/getreportforsupplementarylistForTransferCases?causelist_date='+$scope.causelist_date+'&fromCourt='+$scope.fromCourt);
		response.success(function(data, status, headers, config) {		
			$scope.listData= data.modelList; 
			console.log("Dataaaaaaaa",$scope.listData.length);
...
if($scope.listData.length != 0){
	...
	function getFreshMentionListsForTransferCases() {
		var response = $http.get(urlBase+'ecourt/getFreshMentionListForTransferCases?causelist_date='+$scope.causelist_date+'&fromCourt='+$scope.fromCourt);
		response.success(function(data, status, headers, config) {		
			$scope.listData= data.modelList; 
			console.log("Dataaaaaaaa",$scope.listData.length);
...
if($scope.listData.length != 0){
	...
	if($scope.type_id == 7){
		...
		getFreshMentionListsForTransferCases();
	}
	
	
	 $scope.viewList =function(data){
		 console.log("idddddd",data);
		 window.open(urlBase+"causelist/typeForTrasnfer/"+data.cl_list_type_mid+"/"+data.courtMaster.cm_id,"_blank");
...
/*	function getCauseListTypes() {
	...
	if($scope.type_id!='' && $scope.type_id!=null){
		...
		$scope.model.cl_court_no =$scope.fromCourt
		$scope.model.cl_dol=new Date();
		$scope.maxDate = new Date()
		getData($scope.model);
	}
	...
	function getData(){	
		$scope.search=true;
		$http.post(urlBase+'causelist/getCauseListForTransferCases',$scope.model)
	...
	$scope.viewCaseFile=function(data){
		...
		console.log("Cause List ",data);
		  ...
		  window.open(urlBase+"casefile/viewCauseList/"+data.cl_fd_mid+'?cl_court_no='+data.cl_court_no+'&cl_rec_status='+data.cl_rec_status+'&cl_serial_no='+data.cl_serial_no+'&cl_dol='+data.cl_dol+'&cl_list_type_mid='+data.cl_list_type_mid+'&nPflag='+nPflag,"_blank");
	  }
