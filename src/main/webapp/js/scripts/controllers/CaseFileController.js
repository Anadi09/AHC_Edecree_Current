...
$scope.response=[];
	...
	//============================= Associated Implementation project====================================================
	...
	$scope.sendSingleSms = function(row) {
		...
		if (!row.tillDate) {
		       alert("Please select Till Date for: " + row.name);
		       return;
		   }
	    ...
	    if (!payload.mobile || payload.mobile.length !== 10) {
	        alert("Invalid mobile for: " + payload.name);
	        return;
	    }

	    console.log("Sending payload:", payload);

	    // SEND ONLY CLEAN DATA
	...
	//================================ implementation of extra advocate logic=========================================================
	// MASTER CHECKBOX
	$scope.selectAllExtra = false;

	// Toggle all rows
	$scope.toggleAllExtra = function () {
	    angular.forEach($scope.extraAdvList, function (row) {
	        row.selected = $scope.selectAllExtra;
	    });
	};

	// Check if any selected
	$scope.hasAnySelectedExtra = function () {
	    return $scope.extraAdvList && $scope.extraAdvList.some(function (adv) {
	        return adv.selected;
	    });
	};

	// Sync header checkbox (AUTO CHECK / UNCHECK)
	$scope.syncSelectAllExtra = function () {

	    if (!$scope.extraAdvList || $scope.extraAdvList.length === 0) {
	        $scope.selectAllExtra = false;
	        return;
	    }

	    var allSelected = $scope.extraAdvList.every(function (adv) {
	        return adv.selected;
	    });

	    var noneSelected = $scope.extraAdvList.every(function (adv) {
	        return !adv.selected;
	    });

	    if (allSelected) {
	        $scope.selectAllExtra = true;
	...
	// Send SMS to single row
		$scope.sendSingleSmsExtra = function(adv) {
			
			
			if (!adv.tillDateEx) {
					       alert("Please select Till Date for: " + adv.name);
					       return;
					   }
	...
	$scope.sendAllSmsExtra = function () {

	    var selectedList = $scope.extraAdvList.filter(adv => adv.selected);
	    ...
	    var sourceDate = selectedList.find(adv => adv.tillDateEx)?.tillDateEx;
	    ...
	    //  CREATE CLEAN PAYLOAD (IMPORTANT)
	    var cleanList = selectedList.map(function (adv) {

	        var tillDate = adv.tillDateEx ? adv.tillDateEx : sourceDate;
	    ...
	    for (var i = 0; i < cleanList.length; i++) {
	        if (!cleanList[i].mobile || cleanList[i].mobile.length !== 10) {
	            alert("Invalid mobile for: " + cleanList[i].name);
	            return;
	        }
	    }

	    //  SEND CLEAN DATA ONLY
	    $http.post(urlBase + 'sms/sendAdvAllSms', cleanList)
	        .then(function (res) {
	...
	/* ============================= add extra advocate method================================*/
	
	
	$scope.loadgetadvocate = function () {

	    console.log("Loading extra advocates...");
	    ...
	    $http.post(urlBase + 'sms/getExtraAdvocate', payload)
	        .then(function (res) {
	            $scope.extraAdvList = res.data;
	            console.log("Loaded:", $scope.extraAdvList);
	        })
	        .catch(function (err) {
	            console.error(err);
	        });
